...
void loop()
{
	...
	if (packetSize)
	{
		...
		{
			replyBufferSize += configuration.set(replyBuffer + replyBufferSize, (byte *)udpPacket._payload);
		}
		else if (udpPacket._command == DEVICE_COMMAND_GET_CONTROLLER)
		{
			...
			{
				// memcpy(replyBuffer + 3, sswitch1.toByteArray(), sswitch1.sizeOfDPPayload());
